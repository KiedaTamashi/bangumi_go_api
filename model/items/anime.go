...
type AnimeBgm struct {
	...
	BroadcastDate        time.Time `json:"broadcast_date"`              //放送日期
	BroadcastWeek        WeekNo    `json:"broadcast_week,omitempty"`    //放送星期
	BroadcastTvSite      string    `json:"broadcast_tv_site,omitempty"` //放送电视台
...
func (ani *AnimeBgm) UnmarshalJSON(data []byte) error {
	tmp := struct {
		//Nested
		Id       int            `json:"id,omitempty"`
		Type     int            `json:"type,omitempty"`
	...
	for _, box := range tmpNest.Boxes {
		...
		case "放送开始":
			...
			(ani).BroadcastDate, _ = time.Parse("2006-01-02 15:04:05", fmt.Sprintf("%4s-%2s-%2s", yearMonthDay[0], yearMonthDay[1], yearMonthDay[2])+" 00:00:00")
		case "放送星期":
			(ani).BroadcastWeek = WeekNo(box.Value.(string))
		case "官方网站":
			(ani).OfficialWebSite = box.Value.(string)
		case "播放电视台":
			(ani).BroadcastTvSite = box.Value.(string)
		case "其他电视台":
